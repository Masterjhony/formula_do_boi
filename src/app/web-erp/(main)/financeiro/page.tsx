        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="mt-2 flex items-center gap-2 text-sm text-[#888] font-medium tracking-wider uppercase">
                        Gestão de contas, receitas e despesas
                    </p>
                </div>
                <div className="flex gap-3">
                    <button className="flex items-center gap-2 px-5 py-2.5 bg-[#111] border border-[#333] hover:border-emerald-500/50 text-white text-sm font-semibold rounded-xl transition-all shadow-lg hover:shadow-emerald-500/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-gradient-to-br from-[#1A1A1A] to-[#0A0A0A] border border-[#2A2A2A] hover:border-[#B8860B]/40 rounded-2xl p-6 shadow-xl transition-all duration-300 group relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <p className="text-xs font-bold text-[#888] uppercase tracking-widest">Saldo Consolidado</p>
                        <h3 className="text-4xl font-extrabold text-white mt-2 tracking-tight">{formatCurrency(totalBalance)}</h3>
                ...
                <div className="bg-gradient-to-br from-[#1A1A1A] to-[#0A0A0A] border border-[#2A2A2A] hover:border-[#B8860B]/40 rounded-2xl p-6 shadow-xl transition-all duration-300 group relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <p className="text-xs font-bold text-[#888] uppercase tracking-widest">Receitas (Mês)</p>
                        <h3 className="text-4xl font-extrabold text-white mt-2 tracking-tight">R$ 0,00</h3>
                        <div className="mt-4 text-xs font-semibold text-[#666] uppercase tracking-widest">
                            0 títulos previstos
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-[#1A1A1A] to-[#0A0A0A] border border-[#2A2A2A] hover:border-[#B8860B]/40 rounded-2xl p-6 shadow-xl transition-all duration-300 group relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <p className="text-xs font-bold text-[#888] uppercase tracking-widest">Despesas (Mês)</p>
                        <h3 className="text-4xl font-extrabold text-white mt-2 tracking-tight">R$ 0,00</h3>
                        <div className="mt-4 text-xs font-semibold text-[#666] uppercase tracking-widest">
                            0 contas a pagar
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-[#0F0F0F] rounded-2xl border border-[#222] shadow-xl overflow-hidden">
                        <div className="p-5 border-b border-[#222] flex justify-between items-center bg-[#141414]">
                            <h3 className="font-bold text-white uppercase tracking-wider text-sm">Contas Bancárias</h3>
                            <button className="text-[#D4AF37] hover:text-white p-1.5 rounded-lg hover:bg-[#D4AF37]/20 transition-all">
                        ...
                        <div className="p-3 space-y-2">
                            {accounts && accounts.length > 0 ? (
                                accounts.map((account) => (
                                    <div key={account.id} className="p-4 rounded-xl bg-[#111] hover:bg-[#1A1A1A] transition-all flex items-center justify-between group cursor-pointer border border-[#222] hover:border-[#B8860B]/40">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] text-[#666] uppercase tracking-widest mt-0.5">{account.type === 'checking' ? 'Conta Corrente' : account.type}</p>
                                            </div>
                                        </div>
                                ...
                                <div className="p-8 text-center text-sm text-[#555] uppercase tracking-widest font-bold">Nenhuma conta cadastrada</div>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2">
                    <div className="bg-[#0F0F0F] rounded-2xl border border-[#222] shadow-xl overflow-hidden flex flex-col h-full">
                        <div className="p-5 border-b border-[#222] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#141414]">
                            <h3 className="font-bold text-white uppercase tracking-wider text-sm">Últimas Movimentações</h3>
                            <div className="flex gap-2 w-full sm:w-auto">
                                <div className="relative flex-1 sm:w-64 group/search">
                                    ...
                                    <input 
                                        type="text" 
                                        placeholder="Buscar..." 
                                        className="w-full pl-9 pr-4 py-2.5 text-sm bg-[#0A0A0A] border border-[#222] rounded-xl focus:outline-none focus:border-[#B8860B]/50 text-white placeholder:text-[#444] transition-all"
                                    />
                                </div>
                                <button className="p-2.5 border border-[#222] rounded-xl text-[#888] hover:text-white hover:bg-[#1A1A1A] transition-colors">
                                    ...
                                    {transactions && transactions.length > 0 ? (
                                        transactions.map((tx: any) => (
                                            <tr key={tx.id} className="border-b border-[#1A1A1A] hover:bg-[#141414] transition-colors">
                                                <td className="px-6 py-4 text-[#888] font-mono text-xs">{formatDate(tx.transaction_date)}</td>
                                                <td className="px-6 py-4 font-bold text-white">{tx.description || '-'}</td>
                                                <td className="px-6 py-4 text-[#888]">
                                                    {tx.account?.name || '-'}
                                                </td>
                                                <td className={`px-6 py-4 text-right font-extrabold ${tx.type === 'income' ? 'text-emerald-500' : tx.type === 'expense' ? 'text-rose-500' : 'text-[#888]'}`}>
                                        ...
                                        <tr>
                                            <td colSpan={5} className="px-6 py-16 text-center text-[#555] font-bold uppercase tracking-widest text-xs">
