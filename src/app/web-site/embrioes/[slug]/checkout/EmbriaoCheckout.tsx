...
import { sertanejoLandingCss } from "../../../sertanejo/sertanejoLandingCss";
import { type Doadora, DOADORAS_CONDICOES } from "@/data/doadoras";

/**
 * Checkout da doadora — espelho fiel do SertanejoCheckout adaptado para
 * pacotes de embriões VIT (12 embriões/pacote · 4 prenhezes garantidas).
 *
 * Reutiliza sertanejoLandingCss para garantir paridade visual completa
 * com o checkout do Sertanejo (tokens --st-*, pricing-card, ck-input, etc.).
 *
 * Tiers: escalação de desconto por volume de pacotes. Os percentuais
 * (5/10/15%) são sugestão padrão — Fabrício pode ajustar editando o array
 * TIERS_FACTORY abaixo. Sertanejo tem 4 faixas (1-99/100-399/400-999/1000+);
 * o paralelo aqui é 1/2/3-4/5+ pacotes (já que 1 pacote = 12 embriões).
 */
...
const TIERS_FACTORY = (precoBase: number): Tier[] => [
    {
        range: "1 pacote · 12 embriões",
        rangeLabel: "1-pacote",
        pacotes: 1,
        embrioes: 12,
        discountPct: 0,
    ...
    {
        range: "2 pacotes · 24 embriões",
        rangeLabel: "2-pacotes",
        pacotes: 2,
        embrioes: 24,
        discountPct: 5,
        note: "5% de desconto",
    ...
    {
        range: "3-4 pacotes · 36-48 embriões",
        rangeLabel: "3-4-pacotes",
        pacotes: 3,
        embrioes: 36,
        discountPct: 10,
        note: "10% de desconto",
    ...
    {
        range: "5+ pacotes · 60+ embriões",
        rangeLabel: "5-pacotes",
        pacotes: 5,
        embrioes: 60,
        discountPct: 15,
        note: "15% de desconto",
...
function getUtmParams() {
    ...
    return {
        ...
        utm_campaign: params.get("utm_campaign") || "embrioes/checkout",
...
export default function EmbriaoCheckout({ doadora }: { doadora: Doadora }) {
    ...
    const TIERS = useMemo(() => TIERS_FACTORY(doadora.precoEmbriao), [doadora.precoEmbriao]);

    const nome = doadora.nomeAbcz ?? doadora.rgd;

    useEffect(() => {
        const id = "embriao-checkout-fonts";
    ...
    const handleSelectTier = (t: Tier) => {
        setSelectedTier(t);
        if (!pacotes) setPacotes(String(t.pacotes));
        setTimeout(() => {
            document.getElementById("reserva")?.scrollIntoView({ behavior: "smooth", block: "start" });
        }, 50);
    };

    const pacotesNum = Number(pacotes) || 0;
    const embrioesTotal = pacotesNum * 12;
    const desconto = selectedTier?.discountPct ?? 0;
    const valorBruto = embrioesTotal * doadora.precoEmbriao;
    const valorDesconto = valorBruto * (desconto / 100);
    const valorEstimado = valorBruto - valorDesconto;
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (rawCpf.length === 11) {
            ...
            cpfFormatado = rawCpf.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5");
        }
        ...
        const data = {
            ...
            doadora: `${doadora.rgd} (${nome})`,
            cruzamento: doadora.cruzamento,
            pacotes: pacotesNum,
            embrioes_total: embrioesTotal,
            valor_estimado: fmtBRL(valorEstimado),
            ...getUtmParams(),
        };
        ...
        try {
            const res = await fetch("/api/checkout-embriao", {
                method: "POST",
            ...
            form.reset();
            setTimeout(() => router.push("/sertanejo/obrigado"), 800);
        } catch {
            if (btn) {
    ...
    // Stock progress fictício — espelha o padrão Sertanejo. Pode virar dado
    // real depois (ex: count de pré-reservas no Supabase).
    const reserved = Math.min(doadora.quantidadeEmbrioes * 2, doadora.quantidadeEmbrioes * 4);
    const total = doadora.quantidadeEmbrioes * 6;
    const pct = useMemo(() => (reserved / total) * 100, [reserved, total]);
        ...
        <div className="sertanejo-v2-wrap" style={{ minHeight: "100vh" }}>
            ...
            <div className="sert-dark-leather" style={{
                ...
                <span className="mono" style={{
                    ...
                    Embrião FIV · Nelore Visual × Fórmula do Boi · Safra 2026
                </span>
            </div>
            ...
            <nav className="sert-dark-leather" style={{
                ...
                <div className="ck-wrap" style={{
                    ...
                    <Link href={`/embrioes/${doadora.slug}`} style={{ display: "flex", alignItems: "center", gap: 12 }} aria-label="Fórmula do Boi — Voltar para a doadora">
                        <img src="/logo_complete.svg" alt="Fórmula do Boi" style={{ height: 30, width: "auto", display: "block" }} />
                    </Link>
                    <Link href={`/embrioes/${doadora.slug}`} className="mono nav-link-back" style={{
            ...
            <section className="ck-section sert-dark-leather">
                <div className="ck-wrap">
                    <TagPill>Safra 2026 · Reserva {doadora.rgd}</TagPill>
                    ...
                    <h1 className="serif-tight" style={{
                        ...
                        Reserve seu <em style={{ color: "var(--st-gold-2)" }}>pacote.</em>
                    </h1>
                    <p style={{
                        fontSize: 17, color: "var(--st-txt)", marginBottom: 56, maxWidth: 720,
                        fontWeight: 300, lineHeight: 1.6,
                    }}>
                        {nome} × {doadora.cruzamento} · Nelore PO · {doadora.classificacaoTop} ·
                        MGTe {doadora.mgte.valor.toString().replace(".", ",")} {doadora.mgte.top}.
                        Pacote mínimo de {DOADORAS_CONDICOES.pacoteMinimo} embriões VIT com{" "}
                        ...
                        <div>
                            ...
                            <div style={{ display: "flex", alignItems: "baseline", gap: 12, flexWrap: "wrap" }}>
                                ...
                                <span style={{ color: "var(--st-txt)", fontSize: 18, fontWeight: 300 }}>
                                    de <span style={{ color: "var(--st-bone)" }}>{total.toLocaleString("pt-BR")}</span> embriões
                                </span>
                            </div>
                        </div>
                    ...
                    <div style={{
                        ...
                        <span className="mono" style={{
                            ...
                            Coleta FIV agendada · Laboratório {DOADORAS_CONDICOES.laboratorio.split(" — ")[0]}
                        </span>
                    </div>
                </div>
            </section>
            ...
            <section className="ck-section sert-dark-leather" style={{ borderTop: "1px solid var(--st-rule-2)" }}>
                <div className="ck-wrap" style={{ textAlign: "center" }}>
                    <TagPill>Tabela de pacotes</TagPill>

                    <h2 className="serif-tight" style={{
                        fontSize: "clamp(36px, 5.5vw, 72px)", color: "var(--st-bone)",
                        marginBottom: 14, fontWeight: 500,
                    }}>
                        Pacote a partir de <em style={{ color: "var(--st-gold-2)" }}>{fmtBRL(doadora.precoEmbriao)}/embrião.</em>
                    </h2>
                    <p style={{
                        fontSize: 16, color: "var(--st-txt)", maxWidth: 620,
                        margin: "0 auto 64px", fontWeight: 300, lineHeight: 1.6,
                    }}>
                        Quanto maior o volume, maior o desconto. {DOADORAS_CONDICOES.pacoteMinimo} embriões
                        por pacote · {DOADORAS_CONDICOES.prenhezesGarantidas} prenhezes garantidas ·{" "}
                        {DOADORAS_CONDICOES.parcelamento}.
                    </p>
                    ...
                    <div className="pricing-grid" style={{
                        ...
                        {TIERS.map((t, i) => {
                            ...
                            const precoEfetivo = doadora.precoEmbriao * (1 - t.discountPct / 100);
                            const precoStr = precoEfetivo.toLocaleString("pt-BR", {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                            });
                            const [reais, centavos] = precoStr.split(",");
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div style={{ marginBottom: 8 }}>
                                        <span className="serif-tight" style={{
                                            fontSize: "clamp(34px, 4vw, 52px)", color: "var(--st-bone)", fontWeight: 600,
                                        }}>R$ {reais}</span>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
                            ...
                            Embriões VIT · {DOADORAS_CONDICOES.laboratorio}.
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
                            <span style={{ color: "var(--st-gold-2)" }}>💳</span>
            ...
            <section id="reserva" className="ck-section sert-dark-leather" style={{ borderTop: "1px solid var(--st-rule-2)" }}>
                <div className="ck-wrap" style={{ maxWidth: 880, textAlign: "center" }}>
                    ...
                    <h2 className="serif-tight" style={{
                        ...
                        Reserve seu pacote direto com<br />
                            ...
                            <div style={{
                                ...
                                <div>
                                    ...
                                    <div className="serif" style={{ fontSize: 18, color: "var(--st-bone)", fontStyle: "italic", marginTop: 2 }}>
                                        {selectedTier.range} · estimado {fmtBRL(valorEstimado)}
                                    </div>
                                </div>
                        ...
                        <div style={{ display: "grid", gap: 22 }}>
                            ...
                            <div className="form-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                                ...
                                <div>
                                    ...
                                    <input className="ck-input" id="cpf" name="cpf" placeholder="000.000.000-00" maxLength={18} onChange={handleCpfInput} required />
                                </div>
                            </div>
                            ...
                            <div className="form-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                                ...
                                <div>
                                    <label className="ck-label" htmlFor="matrizes_receptoras">Matrizes receptoras disponíveis</label>
                                    <input type="number" className="ck-input" id="matrizes_receptoras" name="matrizes_receptoras" placeholder="Ex: 20" min={1} required />
                                </div>
                            </div>
                            ...
                            <div className="form-grid-3" style={{ display: "grid", gridTemplateColumns: "110px 1fr 1fr", gap: 16 }}>
                                ...
                                <div>
                                    <label className="ck-label" htmlFor="pacotes_input">Pacotes para reserva</label>
                                    <input type="number" className="ck-input" id="pacotes_input" name="pacotes_input"
                            ...
                            <div>
                                ...
                                <select id="parcelamento" name="parcelamento" className="ck-input" defaultValue="" required>
                                    ...
                                    <option value="10x-boleto">10× sem juros (1 entrada + 9)</option>
                        ...
                        {pacotesNum > 0 && (
                            <div style={{
                                ...
                                <div className="mono" style={{
                                    ...
                                    color: "var(--st-gold-2)", textTransform: "uppercase",
                                    fontWeight: 600,
                                }}>
                                    {pacotesNum} pacote{pacotesNum > 1 ? "s" : ""} · {embrioesTotal} embriões
                                </div>
                                <div className="serif-tight" style={{
                                    fontSize: 24, color: "var(--st-bone)", fontWeight: 500,
                                }}>
                                    {fmtBRL(valorEstimado)}
                                    {desconto > 0 && (
                                        <span className="mono" style={{
                                            marginLeft: 10, fontSize: 11, color: "var(--st-gold)",
