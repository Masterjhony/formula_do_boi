// Helper do `/api/lead-volante` (POST do checkout /volante).
//
// Toda submissão da LP `/volante` deve virar um card em `/reservas`, além de
// gravar a linha de auditoria na aba `Volante` da planilha "Backup leads".
//
// Espelha src/lib/atacante-reservation.ts — mesma mecânica, dados do touro
// Volante MRA. Diferença comercial: o Volante não tem tabela de preço por
// faixa publicada (sidebar do checkout diz "valores confirmados pelo
// curador"), então unit_price/total_value ficam null e o operador define
...
const PRODUCT_BASE_NAME = 'Volante MRA';
const PRODUCT_CENTRAL = 'Central Bela Vista';
const ORIGIN = 'site-volante';
...
function buildNotes(lead: VolanteLead): string {
    const lines: string[] = [];
...
export function buildVolanteReservationInsert(lead: VolanteLead, opts: BuildOptions) {
    ...
    return {
        ...
        utm_campaign: lead.utm_campaign ?? 'volante',
        ...
        history: [
            {
                at: createdAt,
                kind: 'created',
                text: 'Pré-reserva enviada pelo site /volante.',
            },
        ],
...
export async function insertVolanteReservation(
    supabase: SupabaseClient,
    lead: VolanteLead,
    opts: { position: number },
) {
    const payload = buildVolanteReservationInsert(lead, opts);
    const { data, error } = await supabase
        ...
        .single();
    if (error) {
        console.error('[volante-reservation] insert error', error);
        return null;
    }
    return data;
}

export const VOLANTE_RESERVATION_ORIGIN = ORIGIN;
